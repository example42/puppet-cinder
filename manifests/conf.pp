...
# = Define: cinder::conf
#
# With this define you can manage any cinder configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/cinder/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/cinder/my.conf',
#
# [*ensure*]
...
define cinder::conf (
  ...
  $manage_require = pickx($require, $cinder::config_file_require)
  $manage_replace = pickx($replace, $cinder::config_file_replace)
  $manage_notify  = $notify ? {
    'class_default' => $cinder::manage_config_file_notify,
    default         => $notify,
  }

  file { "cinder_conf_${name}":
    ensure  => $ensure,
    source  => $source,
